...
share: false
related: false
excerpt: 'SciAgentGym 面向真实科研流程构建科学智能体评测基准，覆盖多步工具调用、长链任务执行与可审计评测。已被 ICML 2026 接收（Accept, regular）。'
date: 2026-02-13
...
**当前状态：** 已被 **ICML 2026** 接收（Accept, regular）。
...
该工作已被 ICML 2026 接收（Accept, regular），camera-ready 版本发布后会在此继续补充完整的实验设置、任务分类、错误类型分析与结果解读。
